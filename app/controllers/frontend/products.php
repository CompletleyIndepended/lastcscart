...
//
    ...
    list($employeers, $search) = fn_get_products($params, Registry::get('settings.Appearance.employeers_per_page'));
...
function fn_get_username_staff($user_ids) {
    $user_firstname = db_get_field('SELECT firstname FROM ?:users WHERE user_id = ?i', $user_ids);
    $user_lastname = db_get_field('SELECT lastname FROM ?:users WHERE user_id = ?i', $user_ids);
    return $user_firstname . ' ' . $user_lastname;
}
