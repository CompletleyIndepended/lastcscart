...
function fn_update_department($data, $department_id, $lang_code = DESCR_SL){
    ...
    $employeers_ids = !empty($data['employeers_ids']) ? $data['employeers_ids'] : [];

    fn_department_delete_links($department_id);
    fn_department_add_links($department_id, $employeers_ids);
...
function fn_department_add_links($department_id, $employeers_ids){
    $employeers_ids = explode(',',$employeers_ids[0]);
    if(!empty($employeers_ids)){
        foreach($employeers_ids as $employeers_id){
            db_query("REPLACE INTO ?:departments_links ?e", [
                'employeers_id' => $employeers_id,
                'department_id' => $department_id,
            ]);
        }
    }
